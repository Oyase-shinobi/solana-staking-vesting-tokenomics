...
describe('staking_program', () => {
  ...
  it('Is initialized!', async () => {
    ...
    // await simulateTx(provider.connection, res, superOwner.publicKey);
...
export const simulateTx = async (conn: Connection, tx: Transaction, user: PublicKey) => {
  tx.recentBlockhash = (await conn.getLatestBlockhash()).blockhash;
  tx.feePayer = user;
  let res = await conn.simulateTransaction(tx.compileMessage());
